...
import {isToday,isBeforeAndSameMonth,isNextMonth,isPrevMonth,isCurrentMonth} from '../../utils/util'
...
class ShopScheduling extends Component {
  ...
  editSchedual = () => {
    ...
    return (
      <div className="shop_scheduling_info">
        <SwitchDate onChange={this.swicthDate} date={checkedDate} />
        <div className="staff_scheduling_info">
          ...
          {staffSchedule.map(item => {
            ...
            return (
              <div className="name_item" key={item.staff_sn}>
                ...
                <div
                  className="edit_morning "
                  onClick={() => this.swicthShiftStatus(item, 0)}
                >
                  <span
                ...
                <div
                  className="edit_night"
                  onClick={() => this.swicthShiftStatus(item, 1)}
                >
                  <span
                ...
                <div
                  className="edit_all"
                  onClick={() => this.swicthShiftStatus(item, 2)}
                >
                  <span
                ...
                <div
                  className="edit_rest"
                  onClick={() => this.swicthShiftStatus(item, 3)}
                >
                  <span
  ...
  renderWorks = count => {
    ...
    let html = []
    for (let i = 1; i <= count; i++) {
      html.push(<span className='work' key={i}></span>)
    }
    return html
  }
  renderDayItem = (value, curr) => {
    ...
    const todayStyle = date === nowDate
      ? 'border:1px solid rgb(253, 208, 0);background:#fff;'
      : ''
    const currentData = this.dealCurrentStyle(date)
    const { edit, borderTop, borderLeft } = currentData
    const curentStyle = `${!borderTop ? 'border-top-color:transparent;' : ''}${
      !borderLeft ? 'border-left-color:transparent;' : ''
    }`
    const activeStyle = `${
      selectedDate.indexOf(date) !== -1
    ...
    template = `<div class='em-calendar-item  isforbid${isforbid} tip${tip ||
      ''} edit${edit}' date=${date} 
  ...
  renderDay = (day,startDateAt) => {
    const prevMonth= isPrevMonth(day,startDateAt);
    const nextMonth= isNextMonth(day,startDateAt);
    let className = classnames('calendar__day', {
      'calendar__day--today': isToday(day),
      'calendar__day--prevtoday': isBeforeAndSameMonth(day),
      // 'calendar__day--selected': this.isSelected(day),
      'calendar__day--nextmonth': nextMonth,
      'calendar__day--prevmonth': prevMonth,
      'calendar__day--currentmonth': isCurrentMonth(day,startDateAt)
    })
    const morSpan = this.renderWorks(morCount, 8)
    const nightSpan = this.renderWorks(nightCount, 8)
    const allworkSpan = this.renderWorks(allworkCount, 8)
    const restSpan = this.renderWorks(restCount, 8)
    return (
      <div
        key={day}
        data-value={day}
      ...
      >
          ...
          <div> <div className="morwork">{morSpan}</div>
          <div className="nightwork">{nightSpan}</div>
          <div className="allwork">{allworkSpan}</div>
          <div className="restday">{restSpan}</div></div>}
         
      </div>
    )
  }
  render() {
    const { alertStatus, lockVisible,checkedDate } = this.state;
