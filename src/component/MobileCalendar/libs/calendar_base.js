...
/* eslint-disable  eqeqeq , no-array-constructor ,no-redeclare,no-multi-str ,no-unused-vars,no-throw-literal*/
...
(function () {
    ...
    function Calendar(options) {
        options = window.innerCalendarUtil.extend({}, defaultOptions, options);
    ...
    Calendar.prototype = {
        ...
        initEntry: function (op) {
            var self = this;
            var options = window.innerCalendarUtil.extend({
                year: "",
                month: "",
                day: ""
            }, op);
            ...
            self.refreshData({
                ...
                self.refreshData({
                    ...
                    // const swipercon = document.querySelector(self.options.swiper);
                    // ReactDOM.render()
                    if (swipercon) {
        ...
        refreshData: function (dateObj, callback) {
            ...
            for (var i = 1; i < nowXingQiJi + 1; i++) {
                ...
                tmpInfo.push({
                    ...
                    tip: 'prev',
                    isholiday: self.getHoliday(y + "-" + self.tom(m) + "-" + self.tod(preMonthDay)),
                });
            ...
            if (dayCount == '28') {
                ...
                for (var index = nowXingQiJi + 1, indexindex = 1; index < (28 + nowXingQiJi + 1); index++ , indexindex++) {
                    ...
                    tmpInfo.push({
                        ...
                        isforbid: "1", //当前月可点击
                        isholiday: self.getHoliday(self.nowYear + "-" + self.tom(self.nowMonth) + "-" + self.tod(indexindex)),
            ...
            for (var index2 = daonale + 1, index3 = 1; index2 <= 42; index2++ , index3++) {
                ...
                tmpInfo.push({
                    ...
                    tip: 'next',
                    isholiday: self.getHoliday(y2 + "-" + self.tom(m2) + "-" + self.tod(index3))
                });
        ...
        _addEvent: function () {
            ...
            self.mySwiper = new Swiper(self.options.swiper, {
                loop: false,
                observeParents: true,
                observer: true,
                initialSlide: 1,
                ...
                on: {
                    click: function (e, swiper) {
                        ...
                        // 日期灰色部分不启用
                        var _this = findAppointParent(e.target, 'em-calendar-item');
                        // 日期输出值
        ...
        _judgeDaysByYearMonth: function (y, m) {
            ...
            if (y == undefined || y == null) {
                throw { error: "=====获取当前月份天数时，缺少y参数，未定义！=======" };
            }
            if (m == undefined || m == null) {
                throw { error: "=====获取当前月份天数时，缺少m参数，未定义！=======" };
            }
            var y = parseInt(y);
            var m = parseInt(m);
            if (m == 0) {
